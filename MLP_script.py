...
def main():
    train = True  # Set to False for testing mode.

    if train:
        print("TRAINING MODE")
        data = pd.read_csv('train.csv').to_numpy()
        np.random.shuffle(data)
        # Assuming the first column contains labels and the rest pixel values.
        Y_dev = data[:1000, 0]
        ...
        model = L0 + L1 + L2 + L3
        # To load pre-trained weights, uncomment the following:
        # model.load()
        ...
        while True:
            ...
            if i == -1:
                break
            X = data[i:i + 1, :]
            prediction = model.forward_prop(X)[1][-1]
            print("Prediction:", Network.get_predictions(prediction))
            plot_number(X)
